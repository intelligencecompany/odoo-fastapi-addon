...
@router.get("/api/website.snippet.filter", response_model=List[Model.WebsiteSnippetFilterModel], tags=['website', 'snippet', 'filter'])
async def get_websitesnippetfilter(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
...
async def put_websitesnippetfilter(post_id:int, data:dict, api_key:str = Depends(api_key_header)):
    ...
    result = models.execute_kw(ODOO_DB, uid, api_key, 'website.snippet.filter', 'write', [[post_id], data])
    print(result)
