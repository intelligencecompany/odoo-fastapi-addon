...
@router.get("/api/res.currency.rate", response_model=List[Model.CurrencyRateModel], tags=['res', 'currency', 'rate'])
async def get_currencyrate(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
...
@router.post("/api/res.currency.rate", response_model=Model.CurrencyRateModel, tags=['res', 'currency', 'rate'])
...
@router.put("/api/res.currency.rate/{post_id}", response_model=Dict[str, str], tags=['res', 'currency', 'rate'])
