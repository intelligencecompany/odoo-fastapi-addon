...
@router.get("/api/ir.exports", response_model=List[Model.ExportsModel], tags=["ir"])
async def get_exports(fields:str = '', offset:int = 0, limit:int = 1000, api_key:str = Depends(api_key_header)):
    ...
    if uid:
        ...
        results = Model.ExportsModel.from_execute_kw(results, field_list)
        return results
    else:
        return json.dumps({'status': 'Connection failed'})

@router.put("/api/ir.exports/{post_id}", response_model=Dict[str, str], tags=["ir"])
async def put_exports(post_id:int, fields:Dict[str, Any], api_key:str = Depends(api_key_header)):
    uid, models = get_connection(api_key)
    ...
    if uid:
        result = models.execute_kw(ODOO_DB, uid, api_key, 'ir.exports', 'write', [[post_id], fields])
        print(result)
