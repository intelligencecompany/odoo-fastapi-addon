...
from .schemas import ProductCategoryModel as Model
...
@router.get("/api/product.category", response_model=List[Model.ProductCategoryModel], tags=['product', 'category'])
async def get_productcategory(fields:str = '', offset:int = 0, limit:int = 1000, api_key:str = Depends(api_key_header)):
    ...
    results = models.execute_kw(ODOO_DB, uid, api_key, 'product.category', 'search_read', [[]], {'fields': field_list, 'offset': offset, 'limit': limit})
    if results is None:
...
async def put_productcategory(post_id:int, data:dict, api_key:str = Depends(api_key_header)):
    ...
    result = models.execute_kw(ODOO_DB, uid, api_key, 'product.category', 'write', [[post_id], data])
    print(result)
