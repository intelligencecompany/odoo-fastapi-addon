...
async def get_partnerautocompletesync(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
...
@router.post("/api/res.partner.autocomplete.sync", response_model=Model.PartnerAutocompleteSyncModel, tags=['res', 'partner', 'autocomplete', 'sync'])
...
@router.put("/api/res.partner.autocomplete.sync/{post_id}", response_model=Dict[str, str], tags=['res', 'partner', 'autocomplete', 'sync'])
