...
@router.get("/api/blog.blog", response_model=List[Model.BlogModel], tags=['blog', 'blog'])
async def get_blog(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
...
@router.post("/api/blog.blog", response_model=Model.BlogModel, tags=['blog', 'blog'])
...
@router.put("/api/blog.blog/{post_id}", response_model=Dict[str, str], tags=['blog', 'blog'])
