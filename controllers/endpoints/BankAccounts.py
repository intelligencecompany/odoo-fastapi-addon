...
async def get_bankaccounts(fields:str = '', offset:int = 0, limit:int = 1000, api_key:str = Depends(api_key_header)):
    ...
    if uid:
        ...
        results = Model.BankAccountsModel.from_execute_kw(results, field_list)
        return results
    else:
        return json.dumps({'status': 'Connection failed'})

@router.put("/api/res.partner.bank/{post_id}", response_model=Dict[str, str], tags=["res"])
async def put_bankaccounts(post_id:int, fields:Dict[str, Any], api_key:str = Depends(api_key_header)):
    uid, models = get_connection(api_key)
    ...
    if uid:
        result = models.execute_kw(ODOO_DB, uid, api_key, 'res.partner.bank', 'write', [[post_id], fields])
        print(result)
