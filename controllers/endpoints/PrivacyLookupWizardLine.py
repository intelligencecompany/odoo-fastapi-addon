...
@router.get("/api/privacy.lookup.wizard.line", response_model=List[Model.PrivacyLookupWizardLineModel], tags=["privacy"])
async def get_privacylookupwizardline(fields:str = '', offset:int = 0, limit:int = 1000, api_key:str = Depends(api_key_header)):
...
async def put_privacylookupwizardline(post_id:int, fields:Dict[str, Any], api_key:str = Depends(api_key_header)):
    uid, models = get_connection(api_key)
    ...
    if uid:
        result = models.execute_kw(ODOO_DB, uid, api_key, 'privacy.lookup.wizard.line', 'write', [[post_id], fields])
        print(result)
