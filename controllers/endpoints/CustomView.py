...
from .schemas import CustomViewModel as Model
...
async def get_customview(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
    ...
    try:
        results = models.execute_kw(ODOO_DB, uid, api_key, 'ir.ui.view.custom', 'search_read', [[]], {'fields': field_list, 'offset': offset, 'limit': limit})
...
async def put_customview(post_id:int, data:dict, api_key:str = Depends(api_key_header)):
    ...
    result = models.execute_kw(ODOO_DB, uid, api_key, 'ir.ui.view.custom', 'write', [[post_id], data])
    print(result)
