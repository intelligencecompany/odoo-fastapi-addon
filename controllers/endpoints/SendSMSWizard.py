...
@router.get("/api/sms.composer", response_model=List[Model.SendSMSWizardModel], tags=['sms', 'composer'])
async def get_sendsmswizard(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
...
@router.post("/api/sms.composer", response_model=Model.SendSMSWizardModel, tags=['sms', 'composer'])
...
@router.put("/api/sms.composer/{post_id}", response_model=Dict[str, str], tags=['sms', 'composer'])
