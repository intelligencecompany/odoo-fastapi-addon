...
@router.get("/api/portal.share", response_model=List[Model.PortalSharingModel], tags=['portal', 'share'])
async def get_portalsharing(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
...
async def put_portalsharing(post_id:int, data:dict, api_key:str = Depends(api_key_header)):
    ...
    try:
        result = models.execute_kw(ODOO_DB, uid, api_key, 'portal.share', 'write', [[post_id], data])
    except Exception as e:
        logging.error(str(e))
        return JSONResponse(content={ "error": str(e)}, status_code=500)
