...
@router.get("/api/change.password.own", response_model=List[Model.UserchangeownpasswordwizardModel], tags=['change', 'password', 'own'])
async def get_userchangeownpasswordwizard(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
...
@router.post("/api/change.password.own", response_model=Model.UserchangeownpasswordwizardModel, tags=['change', 'password', 'own'])
...
@router.put("/api/change.password.own/{post_id}", response_model=Dict[str, str], tags=['change', 'password', 'own'])
async def put_userchangeownpasswordwizard(post_id:int, data:dict, api_key:str = Depends(api_key_header)):
    ...
    try:
        result = models.execute_kw(ODOO_DB, uid, api_key, 'change.password.own', 'write', [[post_id], data])
    except Exception as e:
        logging.error(str(e))
        return JSONResponse(content={ "error": str(e)}, status_code=500)
