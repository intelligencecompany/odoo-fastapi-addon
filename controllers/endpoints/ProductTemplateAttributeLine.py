...
@router.get("/api/product.template.attribute.line", response_model=List[Model.ProductTemplateAttributeLineModel], tags=['product', 'template', 'attribute', 'line'])
async def get_producttemplateattributeline(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
...
@router.post("/api/product.template.attribute.line", response_model=Model.ProductTemplateAttributeLineModel, tags=['product', 'template', 'attribute', 'line'])
...
@router.put("/api/product.template.attribute.line/{post_id}", response_model=Dict[str, str], tags=['product', 'template', 'attribute', 'line'])
async def put_producttemplateattributeline(post_id:int, data:dict, api_key:str = Depends(api_key_header)):
    ...
    result = models.execute_kw(ODOO_DB, uid, api_key, 'product.template.attribute.line', 'write', [[post_id], data])
    print(result)
