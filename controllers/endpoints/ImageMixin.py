...
from .schemas import ImageMixinModel as Model
...
async def get_imagemixin(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
    ...
    try:
        results = models.execute_kw(ODOO_DB, uid, api_key, 'image.mixin', 'search_read', [[]], {'fields': field_list, 'offset': offset, 'limit': limit})

        if results is None:
            return JSONResponse(content=[], status_code=204)
    
        results = Model.ImageMixinModel.list_from_execute_kw(results, field_list)
...
async def post_blog(data:dict, api_key:str = Depends(api_key_header)):
    ...
    try:
        id = models.execute_kw(ODOO_DB, uid, api_key, 'image.mixin', 'create', [data])
        results = models.execute_kw(ODOO_DB, uid, api_key, 'image.mixin', 'read', [id])
...
async def put_imagemixin(post_id:int, data:dict, api_key:str = Depends(api_key_header)):
    ...
    try:
        result = models.execute_kw(ODOO_DB, uid, api_key, 'image.mixin', 'write', [[post_id], data])
    except Exception as e:
        logging.error(str(e))
        return JSONResponse(content={ "error": str(e)}, status_code=500)
