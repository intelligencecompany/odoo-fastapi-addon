...
async def get_spreadsheetmixin(fields:str = '', offset:int = 0, limit:int = 1000, api_key:str = Depends(api_key_header)):
    ...
    if uid:
        ...
        results = Model.SpreadsheetmixinModel.from_execute_kw(results, field_list)
        return results
    else:
...
async def put_spreadsheetmixin(post_id:int, fields:Dict[str, Any], api_key:str = Depends(api_key_header)):
    uid, models = get_connection(api_key)
    ...
    if uid:
        result = models.execute_kw(ODOO_DB, uid, api_key, 'spreadsheet.mixin', 'write', [[post_id], fields])
        print(result)
