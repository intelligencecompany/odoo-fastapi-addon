...
async def get_scheduledmessages(fields:str = '', offset:int = 0, limit:int = 1000, api_key:str = Depends(api_key_header)):
...
async def put_scheduledmessages(post_id:int, fields:Dict[str, Any], api_key:str = Depends(api_key_header)):
    uid, models = get_connection(api_key)
    ...
    if uid:
        result = models.execute_kw(ODOO_DB, uid, api_key, 'mail.message.schedule', 'write', [[post_id], fields])
        print(result)
