...
async def get_actionwindowclose(api_key:str = Depends(api_key_header)):
    uid, models = get_connection(api_key)
    if uid:
        results = models.execute_kw(ODOO_DB, uid, api_key, 'ir.actions.act_window_close', 'search_read', [[]])
        if results is None:
            return json.dumps([])
