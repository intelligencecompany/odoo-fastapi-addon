...
from .schemas import ModelAccessModel as Model
...
async def get_modelaccess(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
    ...
    try:
        results = models.execute_kw(ODOO_DB, uid, api_key, 'ir.model.access', 'search_read', [[]], {'fields': field_list, 'offset': offset, 'limit': limit})

        if results is None:
            return JSONResponse(content=[], status_code=204)
    
        results = Model.ModelAccessModel.list_from_execute_kw(results, field_list)
...
async def post_blog(data:dict, api_key:str = Depends(api_key_header)):
    ...
    try:
        id = models.execute_kw(ODOO_DB, uid, api_key, 'ir.model.access', 'create', [data])
        results = models.execute_kw(ODOO_DB, uid, api_key, 'ir.model.access', 'read', [id])
...
async def put_modelaccess(post_id:int, data:dict, api_key:str = Depends(api_key_header)):
    ...
    try:
        result = models.execute_kw(ODOO_DB, uid, api_key, 'ir.model.access', 'write', [[post_id], data])
    except Exception as e:
        logging.error(str(e))
        return JSONResponse(content={ "error": str(e)}, status_code=500)
