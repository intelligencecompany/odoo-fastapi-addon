...
class ThemeAssetModel(BaseModel):
    ...
    bundle: str = Field("", alias="bundle", title="Bundle", description="")
    path: str = Field("", alias="path", title="Path", description="")
    sequence: int = Field(0, alias="sequence", title="Sequence", description="")
    copy_ids: Optional[List[int]] = Field(None, alias="copy_ids", title="Assets using a copy of me", description="")
    ...
    @classmethod
    def list_from_execute_kw(cls, data:List[dict], fields:List[str] = []) -> List['ThemeAssetModel']:
        transformed = []
        schema = ThemeAssetModel.model_json_schema()
