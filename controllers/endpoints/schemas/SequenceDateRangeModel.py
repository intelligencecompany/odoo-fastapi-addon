...
class SequenceDateRangeModel(BaseModel):
    ...
    date_from: str = Field("", alias="date_from", title="From", description="")
    date_to: str = Field("", alias="date_to", title="To", description="")
    number_next: int = Field(0, alias="number_next", title="Next Number", description="Next number of this sequence")
    sequence_id: int = Field(0, alias="sequence_id", title="Main Sequence", description="")
    ...
    @classmethod
    def list_from_execute_kw(cls, data:List[dict], fields:List[str] = []) -> List['SequenceDateRangeModel']:
        transformed = []
        schema = SequenceDateRangeModel.model_json_schema()
