...
class ScheduledActionsModel(BaseModel):
    id: Optional[int] = Field(None, alias="id", title="ID", description="")
    nextcall: str = Field("", alias="nextcall", title="Next Execution Date", description="Next planned execution date for this job.")
    ...
    ir_actions_server_id: int = Field(0, alias="ir_actions_server_id", title="Server action", description="")
    user_id: int = Field(0, alias="user_id", title="Scheduler User", description="")
    ...
    cron_name: Optional[str] = Field(None, alias="cron_name", title="Name", description="")
    active: Optional[bool] = Field(None, alias="active", title="Active", description="")
    interval_number: Optional[int] = Field(None, alias="interval_number", title="Interval Number", description="Repeat every x.")
    interval_type: Optional[Any] = Field(None, alias="interval_type", title="Interval Unit", description="")
    numbercall: Optional[int] = Field(None, alias="numbercall", title="Number of Calls", description="How many times the method is called,\na negative number indicates no limit.")
    doall: Optional[bool] = Field(None, alias="doall", title="Repeat Missed", description="Specify if missed occurrences should be executed when the server restarts.")
    lastcall: Optional[str] = Field(None, alias="lastcall", title="Last Execution Date", description="Previous time the cron ran successfully, provided to the job through the context on the `lastcall` key")
    priority: Optional[int] = Field(None, alias="priority", title="Priority", description="The priority of the job, as an integer: 0 means higher priority, 10 means lower priority.")
    ...
    @classmethod
    def list_from_execute_kw(cls, data:List[dict], fields:List[str] = []) -> List['ScheduledActionsModel']:
        transformed = []
        schema = ScheduledActionsModel.model_json_schema()
