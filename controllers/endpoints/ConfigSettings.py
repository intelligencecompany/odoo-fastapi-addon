...
async def get_configsettings(fields:str = '', offset:int = 0, limit:int = 1000, api_key:str = Depends(api_key_header)):
    ...
    if uid:
        ...
        results = Model.ConfigSettingsModel.from_execute_kw(results, field_list)
        return results
    else:
...
async def put_configsettings(post_id:int, fields:Dict[str, Any], api_key:str = Depends(api_key_header)):
    uid, models = get_connection(api_key)
    ...
    if uid:
        result = models.execute_kw(ODOO_DB, uid, api_key, 'res.config.settings', 'write', [[post_id], fields])
        print(result)
