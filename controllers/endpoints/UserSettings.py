...
@router.get("/api/res.users.settings", response_model=List[Model.UserSettingsModel], tags=['res', 'users', 'settings'])
async def get_usersettings(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
...
@router.post("/api/res.users.settings", response_model=Model.UserSettingsModel, tags=['res', 'users', 'settings'])
...
@router.put("/api/res.users.settings/{post_id}", response_model=Dict[str, str], tags=['res', 'users', 'settings'])
