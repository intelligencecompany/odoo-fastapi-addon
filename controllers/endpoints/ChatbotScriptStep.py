...
from .schemas import ChatbotScriptStepModel as Model
...
async def get_chatbotscriptstep(fields:str = '', offset:int = 0, limit:int = 10, api_key:str = Depends(api_key_header)):
    ...
    try:
        results = models.execute_kw(ODOO_DB, uid, api_key, 'chatbot.script.step', 'search_read', [[]], {'fields': field_list, 'offset': offset, 'limit': limit})

        if results is None:
            return JSONResponse(content=[], status_code=204)
    
        results = Model.ChatbotScriptStepModel.list_from_execute_kw(results, field_list)
...
async def post_blog(data:dict, api_key:str = Depends(api_key_header)):
    ...
    id = models.execute_kw(ODOO_DB, uid, api_key, 'chatbot.script.step', 'create', [data])
    results = models.execute_kw(ODOO_DB, uid, api_key, 'chatbot.script.step', 'read', [id])
...
async def put_chatbotscriptstep(post_id:int, data:dict, api_key:str = Depends(api_key_header)):
    ...
    result = models.execute_kw(ODOO_DB, uid, api_key, 'chatbot.script.step', 'write', [[post_id], data])
    print(result)
